...
| 5 | 成果停留在远端 | 成果 bundle 自动回 Mac；GitHub PR 创建；Web 验收后合并；拒绝关闭 PR | Git bundle 实测；PR 本地集成通过，外部 GitHub 待验收 |
| 6 | 文档与产品目标不一致 | 更新中英使用说明、配置要求、限制与验证记录 | 已更新中英文 README 和逐项验收记录 |

## 本轮取舍

- 一个选定 Node 执行完整流水线；Planner 会拆解有依赖的多个步骤，跨机器依赖调度后续加入。
- Planner、Implement、Code Review、Acceptance 只依赖 AgentDriver；当前注册 CodexDriver。Run 和任务包持久保存 agent_kind，Runner 按类型创建驱动。每个阶段独立会话；同一阶段遇到 Agent 主动提问时保存会话 ID，回答后续跑，并把回答传给后续阶段。远端 Full access 跳过工具权限确认；不要求新增模型服务。
