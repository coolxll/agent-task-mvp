...
| 网页交付合并 | 已实现：可在网页审核后把已审核提交快进合并到选中的本机 Git 目录；隔离集成验证通过，真实项目目录尚待验收 |
| GitHub PR | 可选交付方式；真实 PR #17 已验证推送、创建、head/base 复核、系统内批准合并和 Runner 清理；branch protection 阻挡路径尚未实测 |
...
- 评审、Gate 或验收失败时最多自动修复两轮并完整复验；已持久化的 Worker 在意外退出后最多自动恢复两次。
