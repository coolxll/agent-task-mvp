...
| 网页交付合并 | 已实现：可在网页审核后把已审核提交快进合并到选中的本机 Git 目录；隔离集成与真实项目目录（`agent-task-mvp-protect`，含变脏/分支移动拒绝场景）均已验收，见本机网页合并验收记录 |
| GitHub PR | 可选交付方式；真实 PR #17 已验证推送、创建、head/base 复核、系统内批准合并和 Runner 清理。合并被保护规则拒绝时记录结构化错误并可重试，合并确认后自动删除系统创建的远端任务分支；两条路径已于 2026-09-26 在 `coolxll/agent-task-mvp-protect` 现场验收通过 |
