...
4. 若要验收 GitHub PR：选择你有写权限的 GitHub 项目，检查“配置”中的交付方式为 GitHub、目标分支正确；任务通过检查后确认出现 PR 链接；网页 Approve 后 PR 状态应为 `MERGED`，远端任务分支应被自动删除，远端临时 worktree 应被清理。合并被分支保护拒绝时页面会提示可重试，排除保护原因后重试 Approve 即完成合并与清理。真实主路径已由 PR #17 验证；branch protection 拒绝路径与远端分支自动删除已于 2026-09-26 在 `coolxll/agent-task-mvp-protect` 现场验收（见 GitHub 交付安全边界文档）。
...
- GitHub PR 的推送、创建、head/base 复核、合并和 Runner 清理主路径已由 PR #17 真实验证；branch protection 主动阻挡路径与合并后远端任务分支自动删除已于 2026-09-26 在 `coolxll/agent-task-mvp-protect` 现场验收通过（拒绝分类 `merge_protection`、重试幂等、合并后分支删除、422 按成功处理）。
